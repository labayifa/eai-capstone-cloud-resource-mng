...
AWS_ACCESS_KEY = 'AKIAU6GDZD3MNLTT7TON'
AWS_SECRET_KEY = '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'
...
class LoadAWSCredentialsInput(BaseModel):
    file_path: str = Field(
        description="Path to the JSON file containing AWS credentials")
...
def load_credentials():
    ...
    try:
        with open(CREDENTIALS_FILE, "r") as f:
            ...
            AWS_ACCESS_KEY = USER_CREDENTIALS["aws_credentials"].get(
                "access_key", "")
            AWS_SECRET_KEY = USER_CREDENTIALS["aws_credentials"].get(
                "secret_key", "")
    except FileNotFoundError:
...
vectorStore = MongoDBAtlasVectorSearch(
    collectionData, embeddings, index_name="vector_embedding_index")
...
class DynamoDBTableInput(BaseModel):
    ...
    partition_key: Optional[str] = Field(
        default=None, description="Partition key for the table (required for create)")
    sort_key: Optional[str] = Field(
        default=None, description="Sort key for the table (optional for create)")
...
class CloudWebInput(BaseModel):
    cloud_budget: str = Field(
        description="Your monthly budget to spend on cloud")
    frequency_access: str = Field(
        description="How often do you access the data and write?")
    cloud_other: str = Field(
        description="More details about the cloud storage")
...
class CreateS3BucketInput(BaseModel):
    ...
    action: int = Field(
        description="If 1, create the bucket. If 0, delete the bucket. anything else, do nothing.")
    region: str = Field(description="AWS region (e.g., us-east-1)")
...
class RenameS3BucketInput(BaseModel):
    old_bucket_name: str = Field(
        description="The current name of the S3 bucket")
...
class DefineTermsTool(BaseTool):
    ...
    def _run(self):
        definitions = "\n".join(
            [f"- **{term}**: {definition}" for term, definition in TECHNICAL_TERMS.items()])
...
class DownloadFileFromS3Input(BaseModel):
    bucket_name: str = Field(
        description="The name of the S3 bucket to download the file from")
    access_key: str = Field(description="AWS access key")
    secret_key: str = Field(description="AWS secret key")
    region: str = Field(description="AWS region (e.g., us-east-1)")
...
prompt = ChatPromptTemplate.from_messages(
    [
        (
            ...
            Greet the user: "Welcome! I’m here to help you find the best AWS storage solution tailored to your project needs."
            Immediately load AWS credentials by invoking the 'load_aws_credentials' tool to access the required access key, secret key, and region. Ensure these credentials remain in memory for subsequent operations.
            ...
            Use the api_tool to initiate the deployment and provide real-time feedback.

            7. Dynamic Resource Management:

            Enable users to perform additional actions:
            List resources: Use list_s3_buckets to show available storage options.
            Update resources: Handle operations like renaming or modifying bucket settings dynamically.
            Interact with databases: Support actions for creating or deleting DynamoDB tables or configuring RDS instances.
